#!/usr/bin/env python3
"""v10.4.6 バッチ実行スクリプト - 25社を一社ずつ処理（キーエンス除外）"""
import sys
import os
import time
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  v10.4.6 バッチ実行: {len(COMPANIES)}社")
    print(f"  修正: XBRL乖離→fallback / 非セグメントフィルタ / 桁数異常値除去")
    print(f"        報告セグメント複数列対応 / 地理キーワード拡張")
    ...
    print(f"\n\n{'='*70}")
    print(f"  v10.4.6 バッチ完了サマリー ({total_elapsed/60:.1f}分)")
    print(f"{'='*70}")
