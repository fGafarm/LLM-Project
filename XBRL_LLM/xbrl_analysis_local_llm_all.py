#!/usr/bin/env python3
"""
XBRL決算解説生成スクリプト（全ファイル読み込み版）
- デスクトップのXBRL ZIPファイルから全ての.htmファイルを抽出
- 企業ごとに全ファイルを統合して分析
...
class Config:
    ...
    # 出力ディレクトリ
    OUTPUT_DIR = get_desktop_path() / "xbrl_analysis_results_all"
...
def extract_all_htm_from_company_folder(company_folder: Path) -> list[dict]:
    """企業フォルダ内の全ZIPから全ての.htmファイルを抽出"""
    extracted = []
...
def call_ollama(prompt: str, model: str = None) -> dict:
    ...
    payload = {
        ...
        "options": {
            "temperature": 0.3,
            "num_predict": 2048,
            "num_ctx": 16384,  # コンテキストウィンドウを16Kに拡大
        }
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='XBRL決算解説生成ツール（全ファイル読み込み版）')
    ...
    print("=" * 60)
    print("🚀 XBRL決算解説生成ツール（全ファイル読み込み版）")
    print("=" * 60)
    ...
    for i, (year, company_folder) in enumerate(all_company_folders, 1):
        ...
        # 企業フォルダから全.htmファイルを抽出
        files_data = extract_all_htm_from_company_folder(company_folder)
        
        if not files_data:
            print(f"  ⚠️ .htmファイルなし、スキップ")
            continue
